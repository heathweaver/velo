...
async function checkScheduledEmails(): Promise<void> {
  ...
  for (const email of pending) {
    try {
      ...
      // Route through the provider layer rather than the Gmail client: these
      // rows are scheduled for whichever account created them, and an IMAP
      // account has no Gmail client, so every scheduled send on IMAP failed.
